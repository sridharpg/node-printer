...
#define ADD_V8_STRING_PROPERTY(name, key) if((job->key != NULL) && (*job->key != L'\0'))    \
	...
	//LPTSTR               pPrinterName;
        ...
        //DWORD                Priority;
        result_printer_job->Set(MY_NODE_MODULE_CONTEXT_PRE V8_STRING_NEW_UTF8("priority"), V8_VALUE_NEW(Number, job->Priority));
        //DWORD                Size;
        result_printer_job->Set(MY_NODE_MODULE_CONTEXT_PRE V8_STRING_NEW_UTF8("size"), V8_VALUE_NEW(Number, job->Size));
        //DWORD                Status;
        ...
        for(StatusMapType::const_iterator itStatus = getJobStatusMap().begin(); itStatus != getJobStatusMap().end(); ++itStatus)
        {
            ...
            {
                result_printer_job_status->Set(MY_NODE_MODULE_CONTEXT_PRE i_status++, V8_STRING_NEW_UTF8(itStatus->first.c_str()));
            }
        }
        //LPTSTR               pStatus;
        if((job->pStatus != NULL) && (*job->pStatus != L'\0'))
        {
            result_printer_job_status->Set(MY_NODE_MODULE_CONTEXT_PRE i_status++, V8_STRING_NEW_2BYTES((uint16_t*)job->pStatus));
        }
        result_printer_job->Set(MY_NODE_MODULE_CONTEXT_PRE V8_STRING_NEW_UTF8("status"), result_printer_job_status);
...
#undef ADD_V8_STRING_PROPERTY
        ...
        //DWORD                Position;
        result_printer_job->Set(MY_NODE_MODULE_CONTEXT_PRE V8_STRING_NEW_UTF8("position"), V8_VALUE_NEW(Number, job->Position));
        //DWORD                StartTime;
        result_printer_job->Set(MY_NODE_MODULE_CONTEXT_PRE V8_STRING_NEW_UTF8("startTime"), V8_VALUE_NEW(Number, job->StartTime));
        //DWORD                UntilTime;
        result_printer_job->Set(MY_NODE_MODULE_CONTEXT_PRE V8_STRING_NEW_UTF8("untilTime"), V8_VALUE_NEW(Number, job->UntilTime));
        //DWORD                TotalPages;
        result_printer_job->Set(MY_NODE_MODULE_CONTEXT_PRE V8_STRING_NEW_UTF8("totalPages"), V8_VALUE_NEW(Number, job->TotalPages));
        //SYSTEMTIME           Submitted;
        //DWORD                Time;
        result_printer_job->Set(MY_NODE_MODULE_CONTEXT_PRE V8_STRING_NEW_UTF8("time"), V8_VALUE_NEW(Number, job->Time));
        //DWORD                PagesPrinted;
        result_printer_job->Set(MY_NODE_MODULE_CONTEXT_PRE V8_STRING_NEW_UTF8("pagesPrinted"), V8_VALUE_NEW(Number, job->PagesPrinted));
    }
                                     ...
                                     PrinterHandle& iPrinterHandle)
    {
        ...
        if(!jobs)
        {
            ...
            result_printer_job->Set(MY_NODE_MODULE_CONTEXT_PRE V8_STRING_NEW_UTF8("error"), V8_STRING_NEW_UTF8(error_str.c_str()));
            result_printer_jobs->Set(MY_NODE_MODULE_CONTEXT_PRE 0, result_printer_job);
            return std::string("");
        }
        DWORD dummy_bytes = 0;
        ...
        for(DWORD i = 0; i < totalJobs; ++i, ++job)
        {
            ...
            result_printer_jobs->Set(MY_NODE_MODULE_CONTEXT_PRE i, result_printer_job);
        }
        return std::string("");
    }
    ...
    #undef ADD_V8_STRING_PROPERTY
        ...
        for(StatusMapType::const_iterator itStatus = getStatusMap().begin(); itStatus != getStatusMap().end(); ++itStatus)
        {
            ...
            {
                result_printer_status->Set(MY_NODE_MODULE_CONTEXT_PRE i_status, V8_STRING_NEW_UTF8(itStatus->first.c_str()));
                ++i_status;
            }
        }
        result_printer->Set(MY_NODE_MODULE_CONTEXT_PRE V8_STRING_NEW_UTF8("status"), result_printer_status);
        result_printer->Set(MY_NODE_MODULE_CONTEXT_PRE V8_STRING_NEW_UTF8("statusNumber"), V8_VALUE_NEW(Number, printer->Status));
        //DWORD                Attributes;
        ...
        for(StatusMapType::const_iterator itAttribute = getAttributeMap().begin(); itAttribute != getAttributeMap().end(); ++itAttribute)
        {
            ...
            {
                result_printer_attributes->Set(MY_NODE_MODULE_CONTEXT_PRE i_attribute, V8_STRING_NEW_UTF8(itAttribute->first.c_str()));
                ++i_attribute;
            }
        }
        result_printer->Set(MY_NODE_MODULE_CONTEXT_PRE V8_STRING_NEW_UTF8("attributes"), result_printer_attributes);
        //DWORD                Priority;
        result_printer->Set(MY_NODE_MODULE_CONTEXT_PRE V8_STRING_NEW_UTF8("priority"), V8_VALUE_NEW(Number, printer->Priority));
        //DWORD                DefaultPriority;
        result_printer->Set(MY_NODE_MODULE_CONTEXT_PRE V8_STRING_NEW_UTF8("defaultPriority"), V8_VALUE_NEW(Number, printer->DefaultPriority));
        //DWORD                cJobs;
        //result_printer->Set(V8_STRING_NEW_UTF8("jobs"), V8_VALUE_NEW(Number, printer->cJobs));
        //DWORD                AveragePPM;
        result_printer->Set(MY_NODE_MODULE_CONTEXT_PRE V8_STRING_NEW_UTF8("averagePPM"), V8_VALUE_NEW(Number, printer->AveragePPM));

        //DWORD                StartTime;
        if(printer->StartTime > 0)
        {
            result_printer->Set(MY_NODE_MODULE_CONTEXT_PRE V8_STRING_NEW_UTF8("startTime"), V8_VALUE_NEW(Number, printer->StartTime));
        }
        //DWORD                UntilTime;
        if(printer->UntilTime > 0)
        {
            result_printer->Set(MY_NODE_MODULE_CONTEXT_PRE V8_STRING_NEW_UTF8("untilTime"), V8_VALUE_NEW(Number, printer->UntilTime));
        }
        ...
        if(printer->cJobs > 0)
        {
            ...
            result_printer->Set(MY_NODE_MODULE_CONTEXT_PRE V8_STRING_NEW_UTF8("jobs"), result_printer_jobs);
        }
        return "";
    }
}
...
MY_NODE_MODULE_CALLBACK(getSupportedJobCommands)
{
    ...
    {
        result->Set(MY_NODE_MODULE_CONTEXT_PRE i++, V8_STRING_NEW_UTF8(itJob->first.c_str()));
    }
    MY_NODE_MODULE_RETURN_VALUE(result);
}
...
MY_NODE_MODULE_CALLBACK(getSupportedPrintFormats)
{
    ...
    for(DWORD processor_i = 0; processor_i < processorsNum; ++processor_i, ++pProcessor) {
        ...
        for(DWORD j = 0; j < dataTypesNum; ++j, ++pDataType) {
            result->Set(MY_NODE_MODULE_CONTEXT_PRE format_i++, V8_STRING_NEW_2BYTES((uint16_t*)(pDataType->pName)));
        }
    }
